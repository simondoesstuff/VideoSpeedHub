var videoSpeedHubEnableKeybinds = true;
                ...
                chrome.storage.local.get("status", function(result) {
                    if (result.status == 1) {
                        ...
                        videoData.videoManager.prototype.initializeControls = function() {
                            var lArr = chrome.runtime.getURL('/icons/l-arrow.png');
                            var rArr = chrome.runtime.getURL('/icons/r-arrow.png');
                            ...
                            speedDownButton.appendChild(lImg);
                            ...
                            speedUpButton.appendChild(rImg);
                        ...
                        $(document).keydown(function(event) {
                            if (!event.ctrlKey && !event.altKey && !event.shiftKey) {
                                if (event.target.nodeName !== "INPUT" && event.target.nodeName !== "TEXTAREA" && !event.target.isContentEditable) {
                                    if (videoSpeedHubEnableKeybinds) {
                                        if (event.keyCode == 68) {
                                            handleClick('faster');
                                        }
                                        if (event.keyCode == 83) {
